...
import brml.*
load LearnBayesNetData;
figure
pot=setpotclass(pot,'array');
A = dag(pot);
[xcord,ycord]=draw_layout(A);
title('oracle');
V=size(A,1);
data=data(:,1:1000);
Alearn=brml.learnBayesNet(data,ancestralorder(A),nstates,2*ones(1,V),1); %2*ones(1,V)==nstates
figure;
draw_layout(Alearn,cellstr(int2str((1:V)')),zeros(1,V),xcord,ycord);
