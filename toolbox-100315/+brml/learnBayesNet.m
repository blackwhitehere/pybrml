...
for vind=1:length(vars)
    v=vars(vind);
    bestscore=BDscore(data(v,:),[],nstates(v),[],U);
    for nparents=1:maxNparents(v)
        parentset=brml.mynchoosek(vars(vind+1:end),nparents); %in how many ways can parent vars be combined
